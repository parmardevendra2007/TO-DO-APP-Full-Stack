...
const top = () => {
    ...
    const handleForm = async (e) => {
        ...
        setTask('')
        try {
            await axios.post("http://localhost:5000/api/todos", { title: task }, { withCredentials: true })
        } catch (error) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await axios.get("http://localhost:5000/api/todos", { withCredentials: true }).then((res) => {
                    let newArray = [...array]
    ...
    async function toggleComplete(id) {
        try {
            const response = await axios.put(`http://localhost:5000/api/todos/${id}/toggle`, {}, { withCredentials: true });
    ...
    async function handleUpdate(e, id) {
        e.preventDefault();
        try {
            const response = await axios.put(`http://localhost:5000/api/todos/${id}`, { title: editText }, { withCredentials: true });
            const updatedTodo = response.data;
            let newArray = array.map((item) => {
